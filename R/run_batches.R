...
.run_batch_3 <- function(
  ...
  batch_id <- paste0("batch_", ids_subset[1])
  DBI::dbExecute(connection, "PRAGMA enable_profiling = 'json'")

  DBI::dbExecute(
    connection,
    glue::glue("PRAGMA profiling_output = 'profile_explosion_{batch_id}.json'")
  )
  # Step 1: Explode spells to one row per person per variable per day
  picard::execute_sql_file(sql = sql_explosion, conn = connection)
  DBI::dbExecute(connection, "PRAGMA disable_profiling")
  DBI::dbExecute(connection, "PRAGMA enable_profiling = 'json'")
  DBI::dbExecute(
    connection,
    glue::glue("PRAGMA profiling_output = 'profile_combine_{batch_id}.json'")
  )
  # Step 2: Combine daily values into multivariate status intervals
  picard::execute_sql_file(sql = sql_combine, conn = connection)
  DBI::dbExecute(connection, "PRAGMA disable_profiling")
  ...
  DBI::dbExecute(
    connection,
    glue::glue(
      "PRAGMA profiling_output = 'profile_mergestatus_{batch_id}.json'"
    )
  )
  merged_coded <- data.table::as.data.table(DBI::dbGetQuery(
    connection,
