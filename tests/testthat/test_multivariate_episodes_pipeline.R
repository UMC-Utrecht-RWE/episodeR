...
testthat::test_that("Multivariate episodes pipeline produces expected output", {
  ...
  testthat::expect_true(file.exists(file.path(data_dir, "D3_UNIVARIATE_EPISODES.csv")))
  testthat::expect_true(file.exists(file.path(data_dir, "D3_MULTIVARIATE_EPISODES.csv")))
  ...
  output_parquet <- file.path(tempdir(), "multi_test_D3_MULTIVARIATE_EPISODES.parquet")
