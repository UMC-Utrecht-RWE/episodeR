...
testthat::test_that("univariate_episodes_pipeline produces correct output across 10 persons/2 variables", {
  ...
  hive_dir <- file.path(tempdir(), "univariate_episodes_hive_blackbox")
...
testthat::test_that("univariate_episodes_pipeline produces identical output whether or not variables are batched", {
  ...
  run_it <- function(batch_flag, batch_size = NULL) {
    ...
    on.exit(DBI::dbDisconnect(con, shutdown = TRUE), add = TRUE)
    DBI::dbWriteTable(con, "D3_CONCEPTS", concepts, overwrite = TRUE)
    hive_dir <- file.path(tempdir(), paste0("uni_batching_equiv_", batch_flag))
    unlink(hive_dir, recursive = TRUE, force = TRUE)
    on.exit(unlink(hive_dir, recursive = TRUE, force = TRUE), add = TRUE)
