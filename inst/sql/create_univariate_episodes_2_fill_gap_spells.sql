...
WITH ranked AS (
    SELECT
        *,
        ROW_NUMBER()
            OVER (PARTITION BY person_id, variable_id ORDER BY start_episode, end_episode)
            AS rank_asc,
        ROW_NUMBER()
            OVER (PARTITION BY person_id, variable_id ORDER BY start_episode DESC, end_episode DESC)
            AS rank_desc,
        LAG(end_episode)
            OVER (PARTITION BY person_id, variable_id ORDER BY start_episode, end_episode)
            AS prev_end
...
SELECT
    ...
    es.start_episode,
    es.end_episode
FROM empty_spaces AS es INNER JOIN study_variables AS sv ON es.variable_id = sv.variable_id
...
SELECT
    ...
    bf.start_episode,
    bf.end_episode
FROM before_first AS bf INNER JOIN study_variables AS sv ON bf.variable_id = sv.variable_id
...
SELECT
    ...
    al.start_episode,
    al.end_episode
FROM after_last AS al INNER JOIN study_variables AS sv ON al.variable_id = sv.variable_id
